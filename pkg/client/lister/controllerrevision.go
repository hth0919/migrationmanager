...
type ControllerRevisionLister interface {
	...
	ConversionList(selector labels.Selector) ([]*appsv1.ControllerRevision, error)
...
func (s *controllerRevisionLister) ConversionList(selector labels.Selector) ([]*appsv1.ControllerRevision, error) {
	temp , err := s.List(selector)
	ret := make([]*appsv1.ControllerRevision, 0,len(temp))
	for _, rev := range temp {
		aprev := new(appsv1.ControllerRevision)
		aprev.TypeMeta = rev.TypeMeta
		aprev.ObjectMeta = rev.ObjectMeta
		aprev.Data = rev.Data
		aprev.Revision = rev.Revision
		ret = append(ret, aprev)
	}
	return ret, err
}
