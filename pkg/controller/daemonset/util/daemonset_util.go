...
// value is returned.
func GetTemplateGeneration(ds *ketiv1.DaemonSet) (*int64, error) {
	annotation, found := ds.Annotations[ketiv1.DeprecatedTemplateGeneration]
	if !found {
		return nil, nil
	}
...
func CreatePodTemplate(template ketiv1.PodTemplateSpec, generation *int64, hash string) ketiv1.PodTemplateSpec {
...
func IsPodUpdated(pod *ketiv1.Pod, hash string, dsTemplateGeneration *int64) bool {
...
func SplitByAvailablePods(minReadySeconds int32, pods []*ketiv1.Pod) ([]*ketiv1.Pod, []*ketiv1.Pod) {
	unavailablePods := []*ketiv1.Pod{}
	availablePods := []*ketiv1.Pod{}
	for _, pod := range pods {
		if podutil.IsPodAvailable((*v1.Pod)(pod), minReadySeconds, metav1.Now()) {
...
func GetTargetNodeName(pod *ketiv1.Pod) (string, error) {
