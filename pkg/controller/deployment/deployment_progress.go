...
import (
	...
	"github.com/hth0919/migrationmanager/pkg/controller/deployment/util"
...
func (r *ReconcileDeployment) syncRolloutStatus(allRSs []*ketiv1.ReplicaSet, newRS *ketiv1.ReplicaSet, d *ketiv1.Deployment) error {
	newStatus := calculateStatus(allRSs, newRS, d)

	// If there is no progressDeadlineSeconds set, remove any Progressing condition.
	if !util.HasProgressDeadline(d) {
		util.RemoveDeploymentCondition(&newStatus, ketiv1.DeploymentProgressing)
	}

	// If there is only one replica set that is active then that means we are not running
	// a new rollout and this is a resync where we don't need to estimate any progress.
	// In such a case, we should simply not estimate any progress for this deployment.
	currentCond := util.GetDeploymentCondition(d.Status, ketiv1.DeploymentProgressing)
	...
	if util.HasProgressDeadline(d) && !isCompleteDeployment {
		...
		case util.DeploymentComplete(d, &newStatus):
			...
			condition := util.NewDeploymentCondition(ketiv1.DeploymentProgressing, v1.ConditionTrue, util.NewRSAvailableReason, msg)
		...
		case util.DeploymentProgressing(d, &newStatus):
			...
			condition := util.NewDeploymentCondition(ketiv1.DeploymentProgressing, v1.ConditionTrue, util.ReplicaSetUpdatedReason, msg)
		...
		case util.DeploymentTimedOut(d, &newStatus):
			...
			condition := util.NewDeploymentCondition(ketiv1.DeploymentProgressing, v1.ConditionFalse, util.TimedOutReason, msg)
			util.SetDeploymentCondition(&newStatus, *condition)
		}
	}
	...
	if replicaFailureCond := r.getReplicaFailures(allRSs, newRS); len(replicaFailureCond) > 0 {
		...
		util.RemoveDeploymentCondition(&newStatus, ketiv1.DeploymentReplicaFailure)
	}

	// Do not update if there is nothing new to add.
	if reflect.DeepEqual(d.Status, newStatus) {
		// Requeue the deployment if required.
		r.requeueStuckDeployment(d, newStatus)
		return nil
	}
...
func (r *ReconcileDeployment) getReplicaFailures(allRSs []*ketiv1.ReplicaSet, newRS *ketiv1.ReplicaSet) []ketiv1.DeploymentCondition {
	var conditions []ketiv1.DeploymentCondition
	if newRS != nil {
		for _, c := range newRS.Status.Conditions {
			if c.Type != ketiv1.ReplicaSetReplicaFailure {
				continue
			}
...
func (r *ReconcileDeployment) requeueStuckDeployment(d *ketiv1.Deployment, newStatus ketiv1.DeploymentStatus) time.Duration {
	currentCond := util.GetDeploymentCondition(d.Status, ketiv1.DeploymentProgressing)
