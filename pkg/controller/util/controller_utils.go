...
func (r *ControllerExpectations) DeleteExpectations(controllerKey string) {
	if exp, exists, err := r.GetByKey(controllerKey); err == nil && exists {
		if err := r.Delete(exp); err != nil {
			klog.Infof("Error deleting expectations for controller %v: %v", controllerKey, err)
		}
	}
}
...
func (r *ControllerExpectations) SatisfiedExpectations(controllerKey string) bool {
	if exp, exists, err := r.GetExpectations(controllerKey); exists {
		if exp.Fulfilled() {
			klog.Infof("Controller expectations fulfilled %#v", exp)
			return true
		} else if exp.isExpired() {
			klog.Infof("Controller expectations expired %#v", exp)
			return true
		} else {
			klog.Infof("Controller still waiting on expectations %#v", exp)
			return false
		}
	} else if err != nil {
		klog.Infof("Error encountered while checking expectations %#v, forcing sync", err)
	} else {
		...
		klog.Infof("Controller %v either never recorded expectations, or the ttl expired.", controllerKey)
	}
...
func (r *ControllerExpectations) SetExpectations(controllerKey string, add, del int) error {
	...
	klog.Infof("Setting expectations %#v", exp)
	return r.Add(exp)
}
...
func (r *ControllerExpectations) LowerExpectations(controllerKey string, add, del int) {
	if exp, exists, err := r.GetExpectations(controllerKey); err == nil && exists {
		...
		klog.Infof("Lowered expectations %#v", exp)
	}
}
...
func (r *ControllerExpectations) RaiseExpectations(controllerKey string, add, del int) {
	if exp, exists, err := r.GetExpectations(controllerKey); err == nil && exists {
		exp.Add(int64(add), int64(del))
		// The expectations might've been modified since the update on the previous line.
		klog.Infof("Raised expectations %#v", exp)
	}
}
...
func (u *UIDTrackingControllerExpectations) ExpectDeletions(rcKey string, deletedKeys []string) error {
	...
	klog.Infof("Controller %v waiting on deletions for: %+v", rcKey, deletedKeys)
	u.uidStoreLock.Lock()
...
func (u *UIDTrackingControllerExpectations) DeletionObserved(rcKey, deleteKey string) {
	...
	if uids != nil && uids.Has(deleteKey) {
		klog.Infof("Controller %v received delete for pod %v", rcKey, deleteKey)
...
func (u *UIDTrackingControllerExpectations) DeleteExpectations(rcKey string) {
	...
	if uidExp, exists, err := u.uidStore.GetByKey(rcKey); err == nil && exists {
		if err := u.uidStore.Delete(uidExp); err != nil {
			klog.Infof("Error deleting uid expectations for controller %v: %v", rcKey, err)
		}
	}
}
...
func (r RealControllerRevisionControl) PatchControllerRevision(namespace, name string, data []byte) error {
	_, err := r.KetiClient.ControllerRevisions(namespace).Patch(name, types.StrategicMergePatchType, data)
	return err
}
...
type PodControlInterface interface {
	...
	CreatePodsWithControllerRef(number int, obname, namespace string, template *v1.PodTemplateSpec, object runtime.Object, controllerRef *metav1.OwnerReference) error
...
func (r RealPodControl) CreatePods(namespace string, template *v1.PodTemplateSpec, object runtime.Object) error {
	return r.createPods(0,"","", namespace, template, object, nil)
}

func (r RealPodControl) CreatePodsWithControllerRef(number int, obname, namespace string, template *v1.PodTemplateSpec, controllerObject runtime.Object, controllerRef *metav1.OwnerReference) error {
	if err := validateControllerRef(controllerRef); err != nil {
		return err
	}
	klog.Infoln("Create Pod ", namespace, "/", template.Name)
	return r.createPods(number, obname, "", namespace, template, controllerObject, controllerRef)
}
...
func (r RealPodControl) CreatePodsOnNode(nodeName, namespace string, template *v1.PodTemplateSpec, object runtime.Object, controllerRef *metav1.OwnerReference) error {
	...
	return r.createPods(0, "",nodeName, namespace, template, object, controllerRef)
}

func (r RealPodControl) PatchPod(namespace, name string, data []byte) error {
	_, err := r.KetiClient.Pods(namespace).Patch(name, types.MergePatchType, data)
	return err
}
func (r RealPodControl) SetZeroCount() {
	r.Number = 0
}
...
func (r RealPodControl) createPods(number int , obname, nodeName, namespace string, template *v1.PodTemplateSpec, object runtime.Object, controllerRef *metav1.OwnerReference) error {
	...
	pod.APIVersion = "keti.migraion/v1"
	if pod.Labels["keti.checkpoint.type"] == "" {
		pod.Labels["keti.checkpoint.type"] = controllerRef.Kind
		pod.Labels["keti.checkpoint.name"] =  obname
	}
	if pod.Name == "" {
		pod.Name = pod.GenerateName + "pod-" + strconv.Itoa(number)
		pod.GenerateName = ""
	}
	klog.Infoln("POD INFO ::::::::::::::::::::::::::::::::::: ", pod.Name)
	newPod, err := r.KetiClient.Pods(namespace).Create(pod)
	if err != nil {
	...
	klog.Infoln("NEW POD INFO ::::::::::::::::::::::::::::::::::: ", newPod.Name)
	accessor, err := meta.Accessor(object)
	if err != nil {
		klog.Errorf("parentObject does not have ObjectMeta, %v", err)
		return nil
	}
	klog.Infof("Controller %v created pod %v", accessor.GetName(), newPod.Name)
...
func (r RealPodControl) DeletePod(namespace string, podID string, object runtime.Object) error {
	...
	klog.Infof("Controller %v deleting pod %v/%v", accessor.GetName(), namespace, podID)
	if err := r.KetiClient.Pods(namespace).Delete(podID, nil); err != nil && !apierrors.IsNotFound(err) {
...
func (f *FakePodControl) SetZeroCount() {}
...
func (f *FakePodControl) CreatePodsWithControllerRef(number int, obname, namespace string, spec *v1.PodTemplateSpec, object runtime.Object, controllerRef *metav1.OwnerReference) error {
	f.Lock()
	defer f.Unlock()
	f.CreateCallCount++
	if f.CreateLimit != 0 && f.CreateCallCount > f.CreateLimit {
...
func (s ByLogging) Less(i, j int) bool {
	...
	// 3. ready < not ready
	if podutil.IsPodReady((*corev1.Pod)(s[i])) != podutil.IsPodReady((*corev1.Pod)(s[j])) {
		return podutil.IsPodReady((*corev1.Pod)(s[i]))
	}
...
func (s ActivePodsWithRanks) Less(i, j int) bool {
	...
	// 3. Not ready < ready
	// If only one of the pods is not ready, the not ready one is smaller
	if podutil.IsPodReady((*corev1.Pod)(s.Pods[i])) != podutil.IsPodReady((*corev1.Pod)(s.Pods[j])) {
		return !podutil.IsPodReady((*corev1.Pod)(s.Pods[i]))
	}
...
func FilterActiveReplicaSets(replicaSets []*v1.ReplicaSet) []*v1.ReplicaSet {
	activeFilter := func(rs *v1.ReplicaSet) bool {
		return rs != nil && *(rs.Spec.Replicas) > 0
	}
	return FilterReplicaSets(replicaSets, activeFilter)
}

type filterRS func(rs *v1.ReplicaSet) bool

// FilterReplicaSets returns replica sets that are filtered by filterFn (all returned ones should match filterFn).
func FilterReplicaSets(RSes []*v1.ReplicaSet, filterFn filterRS) []*v1.ReplicaSet {
	var filtered []*v1.ReplicaSet
	for i := range RSes {
		if filterFn(RSes[i]) {
...
type ControllersByCreationTimestamp []*corev1.ReplicationController
...
type ReplicaSetsBySizeOlder []*v1.ReplicaSet
...
type ReplicaSetsBySizeNewer []*v1.ReplicaSet
...
func AddOrUpdateTaintOnNode(c clientset.Interface, nodeName string, taints ...*corev1.Taint) error {
	if len(taints) == 0 {
		return nil
	}
	firstTry := true
...
// any API calls.
func RemoveTaintOffNode(c clientset.Interface, nodeName string, node *corev1.Node, taints ...*corev1.Taint) error {
	if len(taints) == 0 {
		return nil
	}
...
func PatchNodeTaints(c clientset.Interface, nodeName string, oldNode *corev1.Node, newNode *corev1.Node) error {
...
func getOrCreateServiceAccount(coreClient v1core.CoreV1Interface, namespace, name string) (*corev1.ServiceAccount, error) {
	...
	if _, err := coreClient.Namespaces().Get(namespace, metav1.GetOptions{}); apierrors.IsNotFound(err) {
		if _, err = coreClient.Namespaces().Create(&corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: namespace}}); err != nil && !apierrors.IsAlreadyExists(err) {
			klog.Warningf("create non-exist namespace %s failed:%v", namespace, err)
		}
	}

	// Create the service account
	sa, err = coreClient.ServiceAccounts(namespace).Create(&corev1.ServiceAccount{ObjectMeta: metav1.ObjectMeta{Namespace: namespace, Name: name}})
